...
export const MessageWindow = ({ messageList } : PMessageWidnow) => {
    ...
    const { pinMessageId, dropPinMessageId, getMessageById, setMessageIdToSrollIntoView } = useChatStore();
    const [ pinnedMessage, setPinnedMessage ] = useState<TMessage | undefined>(undefined);
        ...
        <ScrollArea className='flex-grow flex-shrink w-full h-full flex flex-col relative'>
            { pinMessageId &&
                <div className='absolute inset-0 w-full h-10 bg-slate-100 z-40 flex justify-center items-center px-3 py-1 gap-1'>
                    ...
                    <Cross1Icon className='w-3 h-3 hover:cursor-pointer hover:opacity-50 transition-all' onClick={() => dropPinMessageId()} />
                </div>
            }
            <div className={clsx('flex flex-col', pinMessageId && 'pt-12')}>
                { messagesSplittedByDay.map((dateThread, index) => (
                    <DateThread
                        key={index}
                        messageList={dateThread.messageList} 
                    />
                ))}
            </div>
        </ScrollArea>
